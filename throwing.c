...
int move(int playerNumber)
{
    ...
    while (1) {
        ...
        while (1) {
            ...
            //if the navswitch is pushed and this funkit is allowed to
            //shoot, execute throw() function and sent position data
            if (navswitch_push_event_p (NAVSWITCH_PUSH) && state == 1) {
                throw(count);
